	...
	public void draw() {
		...
		switch (graph) {
			case 0:
				...
				case 2:
                float[] angles = new float[rainfall.length];
                float total = 0;
                for (float value : rainfall) {
                    total += value;
                }
                for (int i = 0; i < rainfall.length; i++) {
                    angles[i] = map(rainfall[i], 0, total, 0, TWO_PI); // Calculate abgles for each slice 
                }

                float lastAngle = 0;
                for (int i = 0; i < angles.length; i++) {
                    float hue = map(i, 0, months.length, 0, 255); // Mapping index to hue value
                    fill(hue, 255, 255); // Using HSB color mode
                    arc(width / 2, height / 2, 300, 300, lastAngle, lastAngle + angles[i]); // Draw pie slice
                    lastAngle += angles[i];

                    // Draw month labels slightly outside the chart
                    float midAngle = lastAngle - angles[i] / 2;
                    float radius = 175; // Adjust the radius to position the labels outside the chart
                    float labelX = width / 2 + cos(midAngle) * radius; // Calculate x-coordinate for label
                    float labelY = height / 2 + sin(midAngle) * radius; // Calculate y-coordinate for label
                    textAlign(CENTER);
                    text("Rainfall pie chart", width / 255, 20);
                    stroke(255);
                    fill(255); // Set fill color to white
                    text(months[i], labelX, labelY); // Draw month label
                }
                break;
        }
